	...
	static class DICOM {
		...
		internal class File {
			...
			public File(string filename, bool DoSimpleParse = false) {
				...
				TransferSyntax = DICOM.TransferSyntax.ExplicitVR;
				Load(DoSimpleParse);
			}
			...
			internal void Load(bool DoSimpleParse) {
				...
				while (true) { //Keep reading until start of Pixel Data;  PIXEL_DATA = 0x 7FE0 0010;
					Tag tag = new Tag(this);
					if (tag.Name == "Pixel Data" || tag.GroupElement == 0x7FE00010) { break; }
					if (tag.Name == "TRANSFER_SYNTAX_UID") {
						switch (tag.Value) { 
							case DICOM.ExplicitVRBigEndian:
								EASR = new BigEndianStreamReader(FileStream);
								TransferSyntax = DICOM.TransferSyntax.ExplicitVR;
								break;

							case DICOM.ExplicitVRLittleEndian:
								TransferSyntax = DICOM.TransferSyntax.ExplicitVR;
								break;

							case DICOM.ImplicitVRLittleEndian:
								TransferSyntax = DICOM.TransferSyntax.ImplicitVR;
								break;

							default:
								TransferSyntax = DICOM.TransferSyntax.Undefined;
								break;
						}
